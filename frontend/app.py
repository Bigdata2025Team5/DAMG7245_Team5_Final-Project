...
if submitted:
    with st.spinner("Generating itinerary with hidden gems..."):
        ...
        try:
            ...
            st.success("✅ Itinerary Generated with Hidden Gems!")
...
if "itinerary_html" in st.session_state:
    ...
    # Add tab view for easier navigation
    tab1, tab2 = st.tabs(["Full Itinerary", "Hidden Gems"])
    
    with tab1:
        # This tab is empty because we're already showing the full HTML itinerary above
        st.write("Scroll through the complete itinerary above")
    
    with tab2:
        st.subheader("🌟 Hidden Gems")
        st.info("Explore these local secrets and off-the-beaten-path attractions!")
        
        # Extract hidden gems section from text
        if "generated_itinerary" in st.session_state:
            itinerary_text = st.session_state.generated_itinerary
            hidden_gems_start = itinerary_text.find("HIDDEN GEMS")
            
            if hidden_gems_start != -1:
                hidden_gems_text = itinerary_text[hidden_gems_start:]
                st.markdown(hidden_gems_text)
            else:
                st.write("No hidden gems found in this itinerary.")
